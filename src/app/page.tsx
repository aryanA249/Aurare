...
function resolveHeroVideoSource(videoUrl: string | undefined) {
  if (!videoUrl) {
    return "/videos/landing-hero.mp4";
  }
  ...
  // Convert common Google Drive share links to direct media links for <video> playback.
  const driveMatch = /drive\.google\.com\/file\/d\/([^/]+)/i.exec(trimmed);
  if (driveMatch?.[1]) {
    const directDriveUrl = "https://drive.google.com/uc?export=download&id=" + driveMatch[1];
    return `/api/video-proxy?url=${encodeURIComponent(directDriveUrl)}`;
  }

  const openIdMatch = /[?&]id=([^&]+)/i.exec(trimmed);
  if (trimmed.includes("drive.google.com") && openIdMatch?.[1]) {
    const directDriveUrl = "https://drive.google.com/uc?export=download&id=" + openIdMatch[1];
    return `/api/video-proxy?url=${encodeURIComponent(directDriveUrl)}`;
  }
...
export default function Home() {
  ...
  const heroVideoSrc = resolveHeroVideoSource(content.heroVideoUrl);
    ...
    <main className="flex-1">
      <section className="mx-auto w-full max-w-7xl px-6 pt-10 md:px-10">
        <div className="relative overflow-hidden rounded-[2.2rem]">
          ...
          <div className="absolute inset-0 bg-[linear-gradient(115deg,rgba(248,240,230,0.9)_6%,rgba(248,240,230,0.62)_44%,rgba(24,17,11,0.35)_100%)]" />

          <div className="relative z-10 grid min-h-[84vh] items-end gap-8 px-6 pb-14 pt-10 md:grid-cols-[1.15fr_1fr] md:px-10 md:pb-16">
            <div className="space-y-5">
              <p className="fade-rise text-xs uppercase tracking-[0.24em] text-neutral-600">
              ...
              <p className="fade-rise fade-rise-delay-2 max-w-lg text-base leading-relaxed text-neutral-800 md:text-lg">
                {content.heroSubtext}
              </p>
              <Link
                href="/collections"
